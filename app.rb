...
def validate_state!
  is_valid_state = session.has_key?(:state) \
      and session[:state].length > 0 \
      and session[:state] == params[:state]
  session.delete(:state)
  halt(403, "The state parameter didn't match what was passed in to the Clef button.") unless is_valid_state
end
